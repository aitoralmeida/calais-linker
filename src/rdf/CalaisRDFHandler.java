...
public class CalaisRDFHandler {
	...
	public static String COMPANIES_SPARQL = "./input/sparql/companies.sparql";
	...
	public Vector<Company> getCompanies(String result) throws IOException{
		Vector<QuerySolution> solutions = this.executeQuery(result, CalaisRDFHandler.COMPANIES_SPARQL);
		Vector<Company> results = new Vector<Company>();
		for (Iterator<QuerySolution> iterator = solutions.iterator(); iterator.hasNext();) {
			QuerySolution querySolution = (QuerySolution) iterator.next();
			String name = querySolution.getLiteral("name").getString();
			String shortname = querySolution.getLiteral("shortname").getString();
			String ticker = querySolution.getLiteral("ticker").getString();
			Company company = new Company(name, shortname, ticker);
			results.add(company);
		}
		return results;		
	}
